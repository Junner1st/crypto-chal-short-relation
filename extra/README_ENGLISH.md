...
A Hard version of [Φ2Sin](https://github.com/Junner1st/crypto-chal-phi-2-sin).
...
Unlike Phi2Sin, this challenge uses `window = 2^48`, so directly enumerating
the target account's `k2` is infeasible. The intended approach is to first solve
a bounded modular short relation:
...
2. Build the congruence `a * (m1 * window + k1) == account_id * window + k2 (mod p)`.
3. Use LLL/CVP or equivalent lattice reduction to recover a bounded short-vector candidate `(m1, k1, k2)`.
4. Check that `x1 = m1 * window + k1` and `x2 = account_id * window + k2` satisfy `a*x1 == x2 (mod p)`.
5. Check that the corresponding curve point exists and that witness `z` satisfies `y = z^2 mod p`.
...
`CCCTF{__walkFAAtHHur_u_bec0me_gIant}`
