...
class Curve:
    def __init__(self, field: Field, b: int, aux: int):
        self.F = field
        self.b = b % field.p
        self.aux = aux % field.p
    ...
    def transform(self, P: Optional[Point]) -> Optional[Point]:
        if P is INF:
            return INF
        return Point((self.aux * P.x) % self.F.p, P.y)
